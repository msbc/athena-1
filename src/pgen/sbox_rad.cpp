...
#error "Magnetic fields must be enabled for this problem generator."
#endif
...
// Broadcast profile data to other ranks
  ...
  if (Globals::my_rank == 0) {
    std::cout << "Loaded " << ic_rows << " rows of initial conditions from " << fn
              << std::endl;
  }
  ...
  if (Globals::nranks == 1 && MSBC_DEBUG) {
    std::cout << "Loaded Opal opacity tables from " << kR_fn << " and " << kP_fn
              << std::endl;
  ...
  for (int k = ks; k <= ke; k++) {
    ...
    for (int k0 = 0; k0 < 2; k0++) {
      ...
      if (norm < 0 || norm > 1) {
        std::stringstream msg;
        msg << "### FATAL ERROR in strat.cpp ProblemGenerator" << std::endl
            << "Interpolation error: norm=" << norm << ", z=" << z
            << ", z_ic=" << z_ic(index - 1) << ", z_ic=" << z_ic(index)
      ...
      for (int i = is; i <= ie; i++) {
        ...
        if (MAGNETIC_FIELDS_ENABLED) {
          ...
          if (ifield == 4) {
            ...
            if (i == ie) pfield->b.x1f(k, j, ie + 1) = 0.0;
            if (j == je)
              pfield->b.x2f(k, je + 1, i) = B0 * (std::cos(static_cast<Real>(kx) * x1));
            if (k == ke)
              pfield->b.x3f(ke + 1, j, i) = B0 * (std::sin(static_cast<Real>(kx) * x1));
          }
        ...
        if (NR_RADIATION_ENABLED || IM_RADIATION_ENABLED) {
          for (int ifr = 0; ifr < pnrrad->nfreq; ++ifr) {
            for (int n = 0; n < pnrrad->nang; ++n) {
              pnrrad->ir(k, j, i, ifr * pnrrad->nang + n) =
                  prat * std::pow(Tgas, 4.0) / PI;
            }
...
void MeshBlock::UserWorkInLoop() { return; }
...
void StratOutflowInnerX3(MeshBlock *pmb, Coordinates *pco, AthenaArray<Real> &prim,
                         FaceField &b, Real time, Real dt, int il, int iu, int jl, int ju,
                         int kl, int ku, int ngh) {
  ...
  for (int k = 1; k <= ngh; k++) {
    for (int j = jl; j <= ju; j++) {
      for (int i = il; i <= iu; i++) {
        ...
        prim(IVZ, kl - k, j, i) =
            (prim(IVZ, kl, j, i) >= 0.0) ? 0.0 : prim(IVZ, kl, j, i);
        if (NON_BAROTROPIC_EOS) {
...
// flow.  All other variables are extrapolated into the
// ghost zones with zero slope.
void StratOutflowOuterX3(MeshBlock *pmb, Coordinates *pco, AthenaArray<Real> &prim,
                         FaceField &b, Real time, Real dt, int il, int iu, int jl, int ju,
                         int kl, int ku, int ngh) {
  // Copy field components from last physical zone
  if (MAGNETIC_FIELDS_ENABLED) {
  ...
  for (int k = 1; k <= ngh; k++) {
    for (int j = jl; j <= ju; j++) {
      for (int i = il; i <= iu; i++) {
        ...
        prim(IVZ, ku + k, j, i) =
            (prim(IVZ, ku, j, i) <= 0.0) ? 0.0 : prim(IVZ, ku, j, i);
        if (NON_BAROTROPIC_EOS) {
...
void LoadOpacity(ParameterInput *pin, AthenaArray<Real> &lims,
                 AthenaArray<Real> &opacity) {
  ...
  if (Globals::my_rank == 0) {
    ...
    if (opacity.GetDim1() > 10000 || opacity.GetDim1() < 0 || opacity.GetDim2() > 10000 ||
        opacity.GetDim2() < 0) {
      std::stringstream msg;
      msg << "### FATAL ERROR in strat_rad.cpp ProblemGenerator" << std::endl
          << "Opacity data has unexpected shape: " << opacity.GetDim2() << ", "
          << opacity.GetDim1() << std::endl;
      ATHENA_ERROR(msg);
    }
...
void OpalOpacityTable::LoadOpacity(const char *opacity_file) {
  ...
  if (Globals::my_rank == 0) {
    ...
    if (log_kappa.GetDim1() > 10000 || log_kappa.GetDim1() < 0 ||
      ...
      std::stringstream msg;
      msg << "### FATAL ERROR in strat_rad.cpp ProblemGenerator" << std::endl
          << "Opacity data has unexpected shape: " << log_kappa.GetDim2() << ", "
          << log_kappa.GetDim1() << std::endl;
      ATHENA_ERROR(msg);
    }
  ...
  if (MSBC_DEBUG && Globals::my_rank == 0) {
    ...
    std::cout << "    Table shape: " << log_kappa.GetDim2() << ", " << log_kappa.GetDim1()
              << std::endl;
    std::cout << "    log_T: " << log_T.GetDim1() << ", log_rhoT: " << log_rhoT.GetDim1()
              << std::endl;
    std::cout << "    breaks: " << breaks.size() << std::endl;
    for (unsigned long i = 0; i < breaks.size(); ++i) {
...
Real OpalOpacityTable::GetOpacity(const Real rho, const Real tgas) {
  ...
  if (i == breaks.size() + 1 || i == 0) {
    ...
    if (nt1 < 0 || nt2 < 0) {
      nt1 = 0;
      nt2 = 0;
    } else if (nt1 >= log_T.GetDim1() || nt2 >= log_T.GetDim1()) {
      nt1 = log_T.GetDim1() - 1;
      nt2 = log_T.GetDim1() - 1;
    }
    if (MSBC_DEBUG) {
      if (nt2 < 0 || nt2 >= log_T.GetDim1() || nt1 < 0 || nt1 >= log_T.GetDim1()) {
        std::stringstream msg;
        msg << "### FATAL ERROR in strat_rad.cpp OpalOpacityTable::GetOpacity"
            << std::endl
  ...
  if (MSBC_DEBUG) {
    ...
    if ((logt < log_T(nt1) || logt > log_T(nt2)) && (nt1 != nt2)) {
      std::stringstream msg;
      msg << "### FATAL ERROR in strat_rad.cpp OpalOpacityTable::GetOpacity" << std::endl
          << "Temperature not bracketed: " << logt << ", " << log_T(nt1) << ", "
          << log_T(nt2) << std::endl;
      ATHENA_ERROR(msg);
    }
  }
  ...
  if (MSBC_DEBUG) {
    if (nrhot1 < 0 || nrhot1 >= log_rhoT.GetDim1() || nrhot2 < 0 ||
        nrhot2 >= log_rhoT.GetDim1()) {
      std::stringstream msg;
      msg << "### FATAL ERROR in strat_rad.cpp OpalOpacityTable::GetOpacity" << std::endl
          << "RhoT index out of bounds: " << nrhot1 << ", " << nrhot2 << std::endl;
      ATHENA_ERROR(msg);
    }
    if ((logrhot < log_rhoT(nrhot1) || logrhot > log_rhoT(nrhot2)) &&
        (nrhot1 != nrhot2)) {
      std::stringstream msg;
      msg << "### FATAL ERROR in strat_rad.cpp OpalOpacityTable::GetOpacity" << std::endl
          << "RhoT not bracketed: " << logrhot << ", " << log_rhoT(nrhot1) << ", "
          << log_rhoT(nrhot2) << std::endl;
      ATHENA_ERROR(msg);
    }
  }
  ...
  if (nrhot1 == nrhot2) {
    if (nt1 == nt2) {
      ...
      kappa =
          kappa_t1_rho1 + (kappa_t2_rho1 - kappa_t1_rho1) * (logt - t_1) / (t_2 - t_1);
    } /* end same T*/
  } else {
    if (nt1 == nt2) {
      kappa = kappa_t1_rho1 +
              (kappa_t1_rho2 - kappa_t1_rho1) * (logrhot - rho_1) / (rho_2 - rho_1);
    } else {
    ...
    for (int j = js; j <= je; j++) {
      for (int i = is; i <= ie; i++) {
        for (int freq = 0; freq < pmb->pnrrad->nfreq; freq++) {
          // TODO: msbc: optimize for multi-frequency
